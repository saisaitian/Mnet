...
#################################################################
# Function used to calculate the GO similarity background data
#################################################################
...
#=== HELP FUNCTION =================
### Use the function WANGMETHOD (from GOSEMSIM - Github, 12/9/2017) to get the pairwise similarities between GO terms
# t1 and t2: a list of GO terms, ont: BP (DATA$ont)
...
# Wang_internal and getSV are helpfunctions to calculate the wang similarity score
...
### Function from GOSEMSIM (12/9/2017 - github) to get all GO terms for a gene considering a background data setting (GO, BP) and
### drop codes (here IEA)
...
### We use always the same disease modules to calculate the GO Similarity score. We can create a matrix for each module
### containing the pairwise Wang similarity scores for the GO terms connected to the mdoule and all GO terms connected to the network
# get a list of all GO terms associated to at least one network gene
...
save(GO_Modules_BP,GO_Modules_MF,GO_Modules_CC, file=paste0("./GO_sim/GO_Modules",".Rdata"))
...
save(GO_pairwise_BP,GO_pairwise_MF,GO_pairwise_CC, file=paste0("./GO_sim/GO_pairwise",".Rdata"))

load(file=paste0("./GO_sim/GO_pairwise",".Rdata"))
...
for (i in 1:length(tcm_targets)) {

  print(i)

  drug <- tcm_targets[[i]]
  name <- names(tcm_targets[i])
...
save(RandGO.permuteScore,sig_go,file = './GO_sim/all_tcm_GO.Rdata')
